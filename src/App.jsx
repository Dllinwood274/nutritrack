...
// ── Local date helper — always uses device timezone, never UTC ───────────────
...
setMealIdea(null);
const prompt = `You are a creative chef and nutritionist. Based on these remaining macros for today, suggest ONE specific meal idea with prep instructions. Remaining: ${remaining.calories} kcal, ${remaining.protein}g protein, ${remaining.carbs}g carbs, ${remaining.fat}g fat. Today's meals already eaten: ${todayMeals.map(m => m.name).join(", ") || "nothing yet"}. Respond ONLY with a JSON object (no markdown, no backticks) with these fields: { "name": "Meal Name", "description": "2-sentence description", "prepTime": "X mins", "cookTime": "X mins", "difficulty": "Easy|Medium|Hard", "ingredients": ["item 1","item 2",...], "steps": ["step 1","step 2",...], "macros": { "calories": 0, "protein": 0, "carbs": 0, "fat": 0 }, "tip": "one pro tip" }`.trim();
try {
const res = await fetch(”/api/chat”, {
method: “POST”,
headers: { “Content-Type”: “application/json” },
    ...
    {tab === "ideas" && (
      <div style={{ paddingBottom: 40 }}>
        ...
        {!smartLoading && smartSuggestions?.suggestions?.map((s, idx) => (
          <SuggestionCard key={idx} s={s} idx={idx} />
        ))}
